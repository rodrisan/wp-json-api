...
/*
Controller name: TP
Controller description: Extended introspection methods
*/

class JSON_API_TP_Controller {
  ...
  public function get_post() {
    ...
    if ($post) {
      ...
      if ($previous) {
        $response['previous_url'] = str_replace('data.','',get_permalink($previous->ID));
      }
      if ($next) {
        $response['next_url'] = str_replace('data.','',get_permalink($next->ID));
      }
      return $response;
    } else {
  ...
  public function get_term_posts() {
    global $json_api;
    $category = $json_api->introspector->get_current_term();
    if (!$category) {
      $json_api->error("Not found.");
    }
    $posts = $json_api->introspector->get_posts(array(
      'post_type' => array('post','page','noticias'),
      'tax_query' => array(
          array(
